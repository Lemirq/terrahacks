...
const articles = [
  {
    ...
    description:
      "A beginner's guide to participating in hackathons and make a great impression.",
    link: "https://taikai.network/blog/hackathon-beginners-guide",
  },
  {
    title: "Ultimate 8 Step Guide to Winning Hackathons",
    description:
      "Gary-Yau Chan shares his top tips for winning hackathons after attending 55 of them.",
...
export default async function PrivatePage() {
  ...
  const generateHeading = () => {
    ...
    if (
      userData &&
      userData[0].complete &&
      !["accepted", "rejected"].includes(userData[0].status)
    )
      return "Application submitted";
    if (userData && userData[0].status === "accepted")
      return "Application accepted";
    if (userData && userData[0].status === "rejected")
  ...
  const generateSubHeading = () => {
    if (userData?.length === 0)
      return "You have not started your application yet.";
    if (
      userData &&
      userData[0].complete &&
      !["accepted", "rejected"].includes(userData[0].status)
      ...
      <div className="w-full min-h-screen max-w-5xl mx-auto fc justify-start items-start h-full gap-2">
        <div className="fc gap-3 sm:fr items-start sm:justify-between w-full">
          <h1 className="text-3xl font-bold">
            Welcome, {data.user.user_metadata.first_name}!
          </h1>
          <form>
            <Button
              type="submit"
        ...
        <p className="text-lg">
          We are excited to have you on board for Hack49 2024! We can't wait to
          see what you build. Remember, the best projects are those that solve
          real-world problems. Good luck!
        </p>
        ...
        {userData &&
          userData.length > 0 &&
          userData[0].status === "accepted" && <Accepted />}
        ...
        <section className="w-full py-5 px-5 sm:px-10 fc gap-5 items-start md:fr justify-between rounded-2xl border border-neutral-300/30 bg-neutral-900 mt-5">
          <div className="fc gap-3 items-start w-full">
            ...
            <p className="text-neutral-300">
              ...
              <span className="text-white font-bold">October 10, 2024</span>
            </p>
          </div>
          <Link href="/apply">
        ...
        <section className="sm:px-10  w-full">
          ...
          <div className="fc gap-5">
            {articles.map((article, index) => (
              <div
                key={index}
                className="w-full bg-neutral-900/60 p-5 rounded-xl"
              >
