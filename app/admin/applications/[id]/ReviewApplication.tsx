...
const accessibleTo = ['vihaan@hack49.com', 'arjun@hack49.com', 'prabh@hack49.com', 'rishi@hack49.com'];
...
const ReviewApplication = ({ id, data, url, adminEmail }: ReviewApplicationProps) => {
	const supabase = createClient();
	const router = useRouter();
	const approve = async () => {
		// fetch referral code
		const { data: refData, error: refError } = await supabase.from('referrals').select('*').eq('user_id', id);
