...
export default function AllApps({
  ...
  const renderCell = React.useCallback(
    async (
      ...
      switch (columnKey) {
        ...
        case "review":
          return (
            <p>
              {reviewByUser ? (
                <User
                  description={reviewByUser.email}
                  name={reviewByUser.firstName + " " + reviewByUser.lastName}
                />
              ) : (
                "Not reviewed yet"
              )}
        ...
        case "country":
          return (
            <p>{`${countries.find((c) => c.code === app.country)?.emoji} ${countries.find((c) => c.code === app.country)?.name}`}</p>
          );
        case "created_at":
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (
      ...
      filteredApplications = filteredApplications.filter((user) =>
        ...
        <div className="flex gap-3">
          ...
          <Dropdown>
            <DropdownTrigger className="hidden sm:flex">
              <Button
                endContent={<IoChevronDown className="text-sm" />}
                variant="flat"
              >
                Status
              </Button>
      ...
      >
        <TableHeader columns={columns}>
          {(column) => (
            <TableColumn
              key={column.uid}
              align={column.uid === "review" ? "center" : "start"}
            >
              {column.name}
            </TableColumn>
          )}
        ...
        <TableBody items={filteredItems}>
          {(item: Database["public"]["Tables"]["applications"]["Row"]) => (
            <TableRow
              className="hover:bg-neutral-700 cursor-pointer transition-colors"
              key={item.user_id}
            >
              {(columnKey) => (
                <TableCell>
                  {renderCell(item, columnKey as keyof typeof columns)}
                </TableCell>
              )}
            </TableRow>
          )}
