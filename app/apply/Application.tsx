...
const Application = ({ user }: { user: User }) => {
	...
	const availableSocials = ['GitHub', 'Twitter', 'Instagram', 'LinkedIn', 'Personal Portfolio'];
	...
	const submit = async (data: FormValues) => {
		...
		if (error) {
			...
			const { data: sData, error } = await supabase.from('referrals').select('*').eq('user_id', user.id);
		...
		<TracingBeam className="px-4 pl-6">
			<main className="w-full min-h-screen overflow-hidden relative py-36 fc justify-start">
				...
				<div className="w-full fc items-start max-w-6xl mx-auto">
					...
					<p className="text-neutral-100 font-bold mb-5">Only takes 30 seconds to apply!</p>
              ...
              />
						...
						<Section title="Social" subheading="These aren't required, but they give us a glimpse of who you are.">
							...
							{socials.has('portfolio') && (
								<Input autoComplete="off" {...registerPortfolio} label="Portfolio" placeholder="https://my-portfolio.com" />
							)}
							{socials.has('github') && (
								<Input autoComplete="off" {...registerGithub} label="GitHub" placeholder="https://github.com/elon_musk" />
							)}
							{socials.has('twitter') && (
								<Input autoComplete="off" {...registerTwitter} label="Twitter" placeholder="https://twitter.com/elonmusk" />
							)}
							{socials.has('instagram') && (
								<Input autoComplete="off" {...registerInstagram} label="Instagram" placeholder="https://instagram.com/elonmusk" />
							)}
						...
						<div className="w-full fc gap-3 items-end">
							...
							{Object.keys(errors).length > 0 && <p>Clear all errors to submit the form</p>}
						</div>
					</form>
				</div>
			</main>
