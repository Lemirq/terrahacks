						...
						answer={
							resume ? (
								<a href={resume.url} target="blank">
									<span className="text-blue-400 underline underline-offset-2">{resume.name}</span> Uploaded
								</a>
							) : (
								'N/A'
							)
...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => {
	return (
		<div className="fc gap-5 w-full items-start">
			...
			<div className="grid w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">{children}</div>
		</div>
	);
};
