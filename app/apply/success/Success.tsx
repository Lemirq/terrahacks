...
import { IoArrowBack, IoChevronForward, IoLogoInstagram } from 'react-icons/io5';
...
const Success = ({ user, referralCode }: { user: User; referralCode: Database['public']['Tables']['referrals']['Row'] }) => {
	...
	const applyReferral = async () => {
		...
		if (referralCode && referralCode.code && loweredCode === referralCode.code) {
		...
		const { data: refData, error: refError } = await supabase.from('referrals').select('used').eq('code', loweredCode).single();
		if (refError) {
		...
		// update users table
		const { data: userData, error: userError } = await supabase.from('users').update({ referred_by: loweredCode }).eq('uid', user.id);
		...
		// apply referral code
		const { error } = await supabase.from('referrals').update({ used: newUsed }).eq('code', loweredCode);
		if (error) {
					...
					<div className="w-full max-w-xl mx-auto px-4">
						...
						<ul className="list-disc pl-6 my-4">
							<li>
								...
								<Link href="https://discord.gg/cgBYcqnvVy" target="_blank" rel="noreferrer" className="text-blue-500 underline">
									Join Discord
								</Link>
							</li>
						...
						<p>
							We want to see your excitement! Post on a social media platform and tag our account. We will feature the best posts on our
							social media.
						</p>
						<ul className="fr gap-2 nav-links w-full justify-start my-2 text-white text-2xl">
							...
							<li>
								<a href="https://www.linkedin.com/company/hack49-global/" target="_blank" rel="noopener noreferrer">
									<FaLinkedin />
								</a>
							</li>
							<li>
