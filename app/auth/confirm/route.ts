...
export async function GET(request: NextRequest) {
	...
	if (token_hash && type) {
		...
		if (!error) {
			...
			const c = await makeID(data.user?.user_metadata?.first_name.toLowerCase() ?? '');
			const { error: codeError } = await supabase.from('referrals').insert({ user_id: data.user?.id, code: c });
